...
class Merger():
    ...
    def graph_output(self):
        # Draw the graph
        pos = nx.spring_layout(self.merged_graph.subgraph)
        nx.draw(self.merged_graph.subgraph, pos, with_labels=True, node_color='lightblue', node_size=500, font_size=16, font_weight='bold')
        edge_labels = nx.get_edge_attributes(self.merged_graph.subgraph, 'key')
        nx.draw_networkx_edge_labels(self.merged_graph.subgraph, pos, edge_labels=edge_labels, font_size=12, font_weight='bold')

        plt.savefig("outputs/merged_graph.pdf")

        nx.drawing.nx_agraph.write_dot(self.merged_graph.subgraph, "outputs/merged_graph.dot")
