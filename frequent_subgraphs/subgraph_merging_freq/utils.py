    ...
    for subgraph_file, inds in file_ind_pairs.items():
        ...
        for line in lines:
            if ':' in line:
                ...
                if op_in != "none":
                    # if op_in == "and" or  op_in == "nand" or op_in == "or" or op_in == "xor" or op_in == "xnor" or op_in == "reduce_and" or op_in == "reduce_or" or op_in == "reduce_xor" or op_in == "reduce_xnor"or op_in == "reduce_bool" or op_in == "logic_not":
                    #     op = 'bit_alu'
                    # elif op_in == "smax" or op_in == "umax" or op_in == "sge" or op_in == "uge" or op_in == "gt" or op_in == "ge":
                    #     op = "gte"
                    # elif op_in == "smin" or op_in == "umin" or op_in == "sle" or op_in == "ule" or op_in == "lt" or op_in == "le":
                    #     op = "lte"
                    # elif op_in == "slt" or op_in == "sgt" or op_in == "ult" or op_in == "ugt" or op_in == "eq" or op_in == "eqx":
                    #     op = "sub"
                    # elif op_in == "ashr" or op_in == "lshr" or op_in == "shr" or op_in == "sshr":
                    #     op = "shr"
                    # elif op_in == "sshl":
                    #     op = "shl"
                    ...
                    #     op = "lut"
                    # elif op_in == "logic_or" or op_in == "reduce_or":
                    #     op = "or"
                    # elif op_in == "logic_and" or op_in == "reduce_and":
                    #     op = "and"
                    # elif op_in == "logic_not":
                    #     op = "not"
                    # elif op_in == "reduce_xnor":
                    #     op = "xnor"
                    # elif op_in == "reduce_xor":
                    #     op = "xor"
                    # elif op_in == "divfloor":
                    #     op = "div"
                    # elif op_in == "modfloor":
                    ...
                    # elif op_in == "pmux":
                    #     op = "mux"
                    # elif op_in == "out_self" or op_in == "in_self" or op_in == "wire" or op_in == "pos":
                    #     op = "const"
                    # elif op_in == "sdffe" or op_in == "dffe" or op_in == "dffsr" or op_in == "adff" or op_in == "dff" or op_in == "sdff":
                    #     op = "dff"
                    # else:
                    op = line.split()[2]
...
def add_primitive_ops(subgraphs):
    ...
    for ind, op in enumerate(used_ops):
        ...
        if op not in config.bit_input_ops and "const" not in op:
            ...
            # if op == "and" or op == "nand" or op == "or" or op == "xor" or op == "xnor" or op == "reduce_and" or op == "redeuce_or" or op == "reduce_xor" or op == "reduce_xnor" or op == "reduce_bool"  or op == "logic_not":
            #     op_t = 'bit_alu'
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # if op == "smax" or op == "umax" or op == "sge" or op == "uge" or op == "gt" or op == "ge":
            #     op_t = "gte"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "smin" or op == "umin" or op == "sle" or op == "ule" or op == "lt" or op == "le":
            #     op_t = "lte"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "slt" or op == "sgt" or op == "ult" or op == "ugt" or op == "eq" or op == "eqx":
            #     op_t = "sub"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "ashr" or op == "lshr" or op == "shr" or op == "sshr":
            #     op_t = "shr"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "sshl":
            #     op_t = "shl"
            ...
            #     op_t = "lut"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "sdffe" or op == "dffe" or op == "dffsr" or op == "adff" or op == "dff" or op == "sdff":
            #     op_t = "dff"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # else:
    ...
    for ind, op in enumerate(used_ops):
        if op not in config.bit_input_ops and "const" not in op and op in config.comm_ops:
            ...
            #     op_t = 'bit_alu'
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "smax" or op == "umax" or op == "sge" or op == "uge" or op == "gt" or op == "ge":
            #     op_t = "gte"
            #     op_graph.add_node(str(config.node_counter), op=config.op_types_flipped[op_t], op_config=[config.op_types_flipped[op]])
            # elif op == "smin" or op == "umin" or op == "sle" or op == "ule" or op == "lt" or op == "le":
            #     op_t = "lte"
        ...
        for module in modules.copy():
            ...
            if module['type'] == 'const' or module['type'] == 'bitconst':
                ...
                if inorder:
                    ...
                    modules.remove(module)
                    sort_count = 1000
            # elif  module['type'] in config.dffs:
            #     inorder = True

            #     for in1_item in module["in1"]:
            #         if "reg" in in1_item or "dff" in in1_item:
            ...
            #         sort_count = 1000
                ...
                inorder = True
                if module.get("in0") != None:
                    for in0_item in module["in0"]:
                        if "reg" in in0_item or "dff" in in0_item:
                            inorder = inorder and in0_item in ids
                        else:
                            inorder = inorder and (is_node_input_or_const(subgraph.nodes(data = True)[in0_item]) or in0_item in ids)
                if module.get("in1") != None:
                    for in1_item in module["in1"]:
                        if "reg" in in1_item or "dff" in in1_item:
                            inorder = inorder and in1_item in ids
                        else:
                ...
                if (module['type'] == 'mux' or module['type'] == 'bitmux') and (module.get("in2") != None):
                    for sel_item in module["in2"]:
                        if "reg" in sel_item or "dff" in sel_item:
                ...
                if module.get("in0") != None and module.get("in1") != None:
                    inorder = True
...
def construct_eq(in0, in1, op, absd_count, in2=""):
    ...
    op_str_map["nor"] = "Data(~(UInt(in_0) | UInt(in_1)))"
    op_str_map["xor"] = "Data(UInt(in_0) ^ UInt(in_1))"
    op_str_map["xnor"] = "Data(~(UInt(in_0) ^ UInt(in_1)))"
    op_str_map["nand"] = "Data(~(UInt(in_0) & UInt(in_1)))"
    ...
    op_str_map["gt"] = "Bit(UInt(in_0) > UInt(in_1))"
    op_str_map["sdffe"] = "Data(UInt(in_0) == UInt(in_1))"
    op_str_map["dffe"] = "Data(UInt(in_0) == UInt(in_1))"
    op_str_map["sdff"] = "Data(UInt(in_0) == UInt(in_1))"
    op_str_map["dff"] = "Data(UInt(in_0) == UInt(in_1))"
    op_str_map["true"] = "Data(1)"
...
def gen_verilog():
    ...
    m.compile(f"outputs/verilog/PE", PE, output="yosys-verilog")
